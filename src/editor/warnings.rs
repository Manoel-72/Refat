...
fn collect_entity_warnings(
    project_root: &Path,
    entity: &Entity,
    warnings: &mut Vec<EditorWarning>,
) {
            ...
            Component::Sprite(sprite) => {
                ...
                if texture_path.is_empty() {
                    ...
                    warnings.push(EditorWarning {
                        ...
                        details: format!(
                            "O arquivo '{}' não foi encontrado no projeto.",
                            texture_path
                        ),
                    });
                }
            ...
            Component::Script(script_component) => {
                ...
                if script_path.is_empty() {
                    warnings.push(EditorWarning {
                        ...
                        details: "Associe um arquivo .rs2 ou remova o componente Script RS2."
                            .to_string(),
                    });
                } else if let Some(script_full_path) =
                    script::resolve_script_path(project_root, script_path)
                {
                    match fs::read_to_string(&script_full_path) {
                        ...
                        Err(_) => warnings.push(EditorWarning {
                            ...
                            label: format!("Script inválido em '{}'", entity.name),
                            details: format!(
                                "Falha ao ler o script: {}",
                                script_full_path.display()
                            ),
                        }),
                    }
                } else if let Err(error) =
                    script::validate_script_reference(project_root, script_path)
                {
                    warnings.push(EditorWarning {
                        kind: EditorWarningKind::Script,
                        severity: EditorWarningSeverity::Error,
            ...
            Component::LuaScript(script_component) => {
                ...
                if script_path.is_empty() {
                    warnings.push(EditorWarning {
                        ...
                        details: "Associe um arquivo .lua para preparar a integração da V0.9."
                            .to_string(),
                    });
                } else if let Some(script_full_path) =
                    script::resolve_script_path(project_root, script_path)
                {
                    match fs::read_to_string(&script_full_path) {
                        ...
                        Err(_) => warnings.push(EditorWarning {
                            ...
                            details: format!(
                                "Falha ao ler o script Lua: {}",
                                script_full_path.display()
                            ),
                        }),
                    }
                } else if let Err(error) =
                    script::validate_lua_script_reference(project_root, script_path)
                {
                    warnings.push(EditorWarning {
                        kind: EditorWarningKind::Script,
                        severity: EditorWarningSeverity::Error,
            ...
            Component::Animator(animator) => {
                let resolved_clip = if animator.state_mode {
                    ...
                    animator
                        .states
                        .get(state_name)
                        .map(|state| state.clip.as_str())
                        .unwrap_or(animator.current.trim())
                } else {
                    ...
                    .unwrap_or(true)
                {
                    warnings.push(EditorWarning {
                        ...
                        details: format!(
                            "O clip '{}' não possui frames configurados.",
                            resolved_clip
                        ),
                    });
                }
            ...
            Component::TextLabel(label) => {
                if label.text.trim().is_empty() {
                    warnings.push(EditorWarning {
                        ...
                        details: "Defina um texto para o HUD ou diálogo aparecer no runtime."
                            .to_string(),
                    });
                }
            }
            Component::UIButton(button) => {
                ...
                if button.width < 72.0 || button.height < 28.0 {
                    warnings.push(EditorWarning {
                        ...
                        details: "Para clique confortável, use pelo menos 72x28 no botão."
                            .to_string(),
                    });
                }
                if button.target_scene.trim().is_empty() && !button.close_runtime {
                    ...
                    if !direct.exists() && !scenes_dir.exists() && !alt_scene.exists() {
                        warnings.push(EditorWarning {
                            ...
                            severity: EditorWarningSeverity::Warning,
                            label: format!(
                                "UIButton com target_scene inválido em '{}'",
                                entity.name
                            ),
                            details: format!(
                                "A cena '{}' não foi encontrada em caminhos comuns do projeto.",
                                raw
                            ),
                        });
                    }
    ...
    for entry in entries.flatten() {
        ...
        if !valid {
            warnings.push(EditorWarning {
                ...
                details:
                    "O JSON do prefab não pôde ser interpretado como Prefab nem como Entity legacy."
                        .to_string(),
            });
        }
    }
