...
impl EditorApp {
    ...
    pub fn start_build_standalone_pc(&mut self) -> Result<(), String> {
        ...
        standalone_export::preflight_standalone_toolchain(&engine_root, &engine_exe_name)?;
    ...
    fn launch_build_after_dialog(&mut self, selected_output: PathBuf) {
        ...
        let params = standalone_export::StandaloneExportParams {
            engine_root,
            bin_name,
            engine_exe_name,
            project_name: project_name.clone(),
        ...
        thread::spawn(move || {
            let (notice_tx, notice_rx) = mpsc::channel::<standalone_export::StandaloneBuildNotice>();
            let ui_tx = tx.clone();
            let forwarder = thread::spawn(move || {
                while let Ok(n) = notice_rx.recv() {
                    let _ = match n {
                        standalone_export::StandaloneBuildNotice::Step(s) => {
                            ui_tx.send(BuildWorkerMessage::Step(s))
                        }
                        standalone_export::StandaloneBuildNotice::Log(l) => {
                            ui_tx.send(BuildWorkerMessage::Log(l))
                        }
                    };
                }
            });

            let result = standalone_export::export_standalone_pc(&params, &notice_tx);
            drop(notice_tx);
