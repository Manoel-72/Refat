...
#[derive(Debug, Clone)]
struct CachedScriptBehavior {
    modified_at: Option<SystemTime>,
    behavior: ScriptBehavior,
}

fn script_cache() -> &'static Mutex<HashMap<PathBuf, CachedScriptBehavior>> {
    static CACHE: OnceLock<Mutex<HashMap<PathBuf, CachedScriptBehavior>>> = OnceLock::new();
    ...
    if let Ok(cache) = script_cache().lock() {
        if let Some(cached) = cache.get(&full_path) {
            if cached.modified_at == modified_at {
                return Ok(cached.behavior.clone());
            }
        }
    }
