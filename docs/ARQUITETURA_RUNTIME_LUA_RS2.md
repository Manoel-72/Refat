...
Aliases como `game.get_delta_time()` e `game.get_elapsed_time()` ficam apenas como compatibilidade temporária. Os nomes oficiais seguem como funções: `game.delta_time()` e `game.elapsed_time()`.
