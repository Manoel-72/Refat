...
impl EditorApp {
    pub fn new(_cc: &eframe::CreationContext<'_>) -> Self {
        ...
        let initial_scene = Scene::new("Cena Principal");
        ...
        Self {
            ...
            open_scenes: vec![OpenSceneDocument { scene: initial_scene, file_path: None }],
...
fn is_matr_file_path(path: &Path) -> bool {
    ...
    pub fn scene_file_candidates(&self) -> Vec<PathBuf> {
        ...
        let mut files: Vec<PathBuf> = std::fs::read_dir(&scenes_dir)
            ...
            .filter(|path| path.file_name().and_then(|n| n.to_str()).map(|n| n.ends_with(".scene.json")).unwrap_or(false))
            .collect();
        files.sort();
        files
    }
    ...
    pub fn open_scene_from_path(&mut self, path: PathBuf) -> Result<(), String> {
        ...
        if let Some(existing_index) = self.open_scenes.iter().position(|doc| doc.file_path.as_ref() == Some(&path)) {
        ...
        self.clear_entity_selection();
        self.assets.refresh();
        self.status_msg = format!("✅ Cena carregada: {}", path.file_name().and_then(|n| n.to_str()).unwrap_or("cena"));
        Ok(())
    }
    ...
    pub fn close_scene_tab(&mut self, index: usize) {
        ...
        let next_index = self.active_scene_index.min(self.open_scenes.len().saturating_sub(1));
