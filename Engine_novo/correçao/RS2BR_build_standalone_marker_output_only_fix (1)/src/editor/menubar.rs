        ...
        ui.vertical(|ui| {
            ...
            egui::menu::bar(ui, |ui| {
                ...
                ui.menu_button("📁 Arquivo", |ui| {
                    ...
                    if menu_item(ui, "🆕", "Novo Projeto...", "Cria uma pasta de projeto nova").clicked() {
                    ...
                    if menu_item(ui, "📂", "Abrir Projeto...", "Abre um projeto existente pelo caminho").clicked() {
                        app.open_project_dialog = Some(String::new());
                        ui.close_menu();
                    }
                    if menu_item(ui, "📦", "Criar Template Básico", "Gera estrutura inicial de pastas e arquivos").clicked() {
                        app.new_template_dialog = Some("MeuProjeto".to_string());
                        ui.close_menu();
                    }
                    if menu_item(ui, "🏠", "Voltar ao Hub", "Fecha o projeto e volta à tela inicial").clicked() {
                    ...
                    if menu_item(ui, "🎬", "Nova Cena", "Cria uma cena vazia e abre em nova aba").clicked() {
                        app.push_undo_state();
                        let next_name = format!("Cena {}", app.open_scenes.len() + 1);
                        app.create_new_scene_tab(next_name);
                        ui.close_menu();
                    }
                    if menu_item(ui, "💾", "Salvar Cena  Ctrl+S", "Salva a cena ativa em assets/scenes/").clicked() {
                    ...
                    ui.separator();
                    if menu_item(ui, "🔄", "Restaurar Layout", "Volta o layout padrão dos painéis").clicked() {
                        app.layout = crate::editor::EditorLayout::default();
                        app.save_layout_to_disk();
                        ui.close_menu();
                    }
                    if menu_item(ui, "💾", "Salvar Layout", "Salva o tamanho atual dos painéis").clicked() {
                        app.save_layout_to_disk();
                        ui.close_menu();
                    }
                ...
                ui.menu_button("🎮 Criar", |ui| {
                    ...
                    if menu_item(ui, "🖼", "Sprite", "Entidade com componente Sprite para exibir imagens").clicked() {
                        app.push_undo_state();
                    ...
                    if menu_item(ui, "📷", "Câmera", "Define o ponto de visão do jogador na cena").clicked() {
                        app.push_undo_state();
                    ...
                    if menu_item(ui, "🔤", "Texto UI", "Rótulo de texto na interface do jogo").clicked() {
                        app.push_undo_state();
                    ...
                    if menu_item(ui, "🔘", "Botão UI", "Botão clicável para menus e interfaces").clicked() {
                        app.push_undo_state();
                ...
                ui.menu_button("🎬 Cena", |ui| {
                    ...
                    ui.label(egui::RichText::new(format!("{} entidade(s) | {} aba(s) aberta(s)",
                        app.scene.entities.len(), app.open_scenes.len())).small().color(TEXT_DIM));
                    ui.separator();

                    section_label(ui, "COR DE FUNDO");
                    let bg = &mut app.scene.background_color;
                    egui::Grid::new("menu_bg_grid").num_columns(2).spacing([6.0, 3.0]).show(ui, |ui| {
                    ...
                    let (r, _) = ui.allocate_exact_size(egui::vec2(ui.available_width(), 20.0), egui::Sense::hover());
                ...
                ui.menu_button("✏ Editor", |ui| {
                    ...
                    if menu_item(ui, "📋", "Copiar Entidade  Ctrl+C", "Copia a entidade selecionada").clicked() {
                    ...
                    if menu_item(ui, "📌", "Colar Entidade  Ctrl+V", "Cola a entidade copiada com novo ID").clicked() {
                        if let Some(template) = app.entity_clipboard.clone() {
                            ...
                            if let Some(t) = cloned.transform_mut() { t.x += 32.0; t.y -= 32.0; }
                    ...
                    if menu_item(ui, "🗑", "Deletar Seleção  Del", "Remove a entidade selecionada").clicked() {
                ...
                ui.menu_button("❓ Ajuda", |ui| {
                    ...
                    ui.label(egui::RichText::new(format!("Status: {}", crate::core::version::ENGINE_STATUS)).small().color(TEXT_DIM));
                    ui.separator();
                    ...
                    ui.label(egui::RichText::new("  1. Crie ou abra um projeto no Hub").small());
                    ui.label(egui::RichText::new("  2. Use 'Criar' para adicionar entidades à cena").small());
                    ui.label(egui::RichText::new("  3. Selecione uma entidade e veja o Inspector →").small());
                    ui.label(egui::RichText::new("  4. Adicione componentes (Sprite, Script, etc.)").small());
                    ui.label(egui::RichText::new("  5. Salve a cena e pressione ▶ Play para testar").small());
                    ui.label(egui::RichText::new("  6. Duplo clique em script abre no VS Code").small());

                    ui.separator();
                    section_label(ui, "💡 DICAS RÁPIDAS");
                    ui.label(egui::RichText::new("  • Arraste imagens do Asset Browser para a cena").small().color(TEXT_DIM));
                    ui.label(egui::RichText::new("  • Clique direito na Hierarquia para criar filhos").small().color(TEXT_DIM));
                    ui.label(egui::RichText::new("  • MATR = prefab reutilizável em múltiplas cenas").small().color(TEXT_DIM));
                    ui.label(egui::RichText::new("  • Inspector mostra as props da cena se nada selecionado").small().color(TEXT_DIM));
                    ui.label(egui::RichText::new("  • Use Snap 32px para alinhar entidades na grade").small().color(TEXT_DIM));
                });
