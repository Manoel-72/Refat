...
impl EditorApp {
    ...
    pub fn start_build_standalone_pc(&mut self) -> Result<(), String> {
        ...
        let selected_parent = rfd::FileDialog::new()
            .set_title("Escolher pasta de saída da build")
            .set_directory(self.project_root.join("build"))
            .pick_folder()
            .ok_or_else(|| "Build cancelada: nenhuma pasta de saída foi escolhida.".to_string())?;

        let export_root = selected_parent.join(&safe_project_name);
        let build_cache_root = selected_parent.join(format!("_rs2br_build_cache_{}", safe_project_name));
        ...
        thread::spawn(move || {
            ...
            let run = || -> Result<PathBuf, String> {
                ...
                send_log("ℹ O editor continua aberto enquanto a build roda. Na primeira vez pode demorar porque o Cargo baixa dependências.".to_string());
                ...
                let mut child = Command::new("cargo")
                    ...
                    .arg("--bin")
                    .arg(&bin_name)
                    .env("CARGO_TARGET_DIR", &build_cache_root)
                ...
                let exe_src = build_cache_root.join("release").join(&engine_exe_name);
                if !exe_src.exists() {
                    return Err(format!(
    ...
    fn show_build_progress_window(&mut self, ctx: &egui::Context) {
        ...
        egui::Window::new("Build Standalone PC")
            ...
            .show(ctx, |ui| {
                ...
                ui.label(format!("Tempo: {:.1}s", job.started_at.elapsed().as_secs_f32()));
                ui.small("O editor continua aberto. A build não deve mais travar a interface.");
                ui.separator();
                egui::ScrollArea::vertical()
                    ...
                    .show(ui, |ui| {
                        for line in &job.log_lines {
                            let color = if line.contains("error[") || line.contains('❌') || line.to_lowercase().contains("erro") || line.to_lowercase().contains("failed") {
                                egui::Color32::from_rgb(255, 120, 120)
                            } else if line.contains("warning") || line.contains('⚠') {
                                egui::Color32::from_rgb(255, 210, 90)
                            } else if line.contains('✅') || line.to_lowercase().contains("finished") || line.to_lowercase().contains("compiling") || line.to_lowercase().contains("downloading") {
