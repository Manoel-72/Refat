...
impl EditorApp {
    ...
    pub fn start_build_standalone_pc(&mut self) -> Result<(), String> {
        ...
        let build_cache_root = unique_temp_build_dir(&chosen_stem);
        ...
        thread::spawn(move || {
            ...
            let run = || -> Result<PathBuf, String> {
                ...
                copy_engine_workspace(&engine_root, &temp_engine_root)
    ...
    fn show_build_progress_window(&mut self, ctx: &egui::Context) {
        ...
        egui::Window::new("Build Standalone PC")
            .collapsible(false)
            .resizable(false)
            .default_size(egui::vec2(520.0, 140.0))
            .anchor(egui::Align2::CENTER_CENTER, egui::vec2(0.0, 0.0))
            .show(ctx, |ui| {
                ...
                ui.add_space(6.0);
                ui.add(
                    egui::ProgressBar::new(job.progress.clamp(0.0, 1.0))
                        .show_percentage()
...
fn unique_temp_build_dir(project_name: &str) -> PathBuf {
    let timestamp = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .map(|d| d.as_millis())
        .unwrap_or(0);
    std::env::temp_dir().join(format!("rs2br_build_{}_{}", sanitize_filename(project_name), timestamp))
}

fn copy_engine_workspace(src_root: &Path, dst_root: &Path) -> std::io::Result<()> {
    ...
    copy_filtered(src_root, dst_root)
}
