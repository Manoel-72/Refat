...
use crate::{
    core::{project::ProjectConfig, scene::Scene, version},
    runtime::{self, RuntimeState, camera, renderer::{self, UiAction}, context::{RuntimeContext, RuntimePlayState}},
};

pub fn detect_standalone_project_root() -> Option<PathBuf> {
    let exe_dir = env::current_exe().ok()?.parent()?.to_path_buf();
    if exe_dir.join("project.json").exists() && exe_dir.join("assets").exists() {
        Some(exe_dir)
    } else {
        None
    }
...
impl StandaloneApp {
    pub fn new(_cc: &eframe::CreationContext<'_>, project_root: PathBuf) -> Self {
        ...
        let scene = crate::serialization::scene_serializer::try_load_scene_from_path(&scene_path)
            .unwrap_or_else(|| Scene::new(&project_config.name));
