-- Cooldowns como variáveis locais (VM cacheada, persistem entre frames)
local hit_cooldown   = 0.0
local spawn_cooldown = 0.0
local hp_real        = 100   -- rastreamos o HP real aqui para o HUD
...
function on_start()
    ...
    session.set("score", session.get("score", 0))
end
...
function on_update(dt)
    ...
    -- Spawn (só Enter — "E" e demais teclas não mapeadas no engine)
    ...
    if hit_cooldown <= 0.0 then
        if game.collision_enter("Enemy") or game.collision_stay("Enemy") then
            hp_real = math.max(0, hp_real - 10)
            entity.damage(10)
            session.set("player_hp", hp_real)
