        ...
        Some(SceneInteraction::Drag { id, dx, dy }) => {
            ...
            for target_id in target_ids {
                if let Some(e) = app.find_entity_mut(&target_id) {
                    if let Some(Component::Transform(t)) = e.components.get_mut(0) {
                        t.x += dx;
                        t.y += dy;
...
fn draw_entity(
    ...
    let mut collider: Option<&crate::engine::component::BoxCollider> = None;
...
fn create_entity_at(app: &mut EditorApp, kind: QuickCreateKind, x: f32, y: f32) {
    ...
    let mut entity = match kind {
        ...
        QuickCreateKind::Sprite => {
            ...
            entity.add_component(Component::Sprite(crate::engine::component::Sprite::default()));
            entity
        }
        QuickCreateKind::Camera => {
            let mut entity = Entity::new("Camera");
            entity.add_component(Component::Camera2D(crate::engine::component::Camera2D::default()));
            entity
        }
    };
